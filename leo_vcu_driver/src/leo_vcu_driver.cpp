...
void LeoVcuDriver::call_local_mode()
{
  ...
  if (
    operation_mode_cmd_ptr_->is_local_mode_available &&
    operation_mode_cmd_ptr_->mode != autoware_adapi_v1_msgs::msg::OperationModeState::LOCAL) {
...
void LeoVcuDriver::operation_mode_handler()
{
  if (autoware_data_ready()) {
    ...
    if (
      ...
      !operation_mode_cmd_ptr_->is_in_transition && comp_to_llc_cmd.vehicle_signal_cmd.mode == 1) {
      call_local_mode();
      return;
    }
  }
